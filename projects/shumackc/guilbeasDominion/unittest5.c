...
int main()
{
	...
	mineCardEffect(&G1, 0, copper, silver, 0);
	...
	assertInt(0, mineCardEffect(&G1, 0, copper, silver, 0));

	printf("\n\nBegin Testing baronCardEffect() on G2:\n");

	printf("\nReturn Error (-1) Test: ");
	assertInt(-1, mineCardEffect(&G2, 0, -1, silver, 0));
	...
	//*********************BUG WILL BE FOUND HERE************************
	printf("\nReturn Error (-1) Test: ");
	assertInt(-1, mineCardEffect(&G2, 0, copper, minion, 0));
